...
$feedback = new \XoopsModules\Slider\Common\ModuleFeedback();
...
switch ($op) {
    ...
    case 'send':
        ...
        $title       = \constant('CO_SLIDER_FB_SEND_FOR') . $GLOBALS['xoopsModule']->getVar('dirname');
        $body        = \constant('CO_SLIDER_FB_NAME') . ': ' . $your_name . '<br>';
        $body        .= \constant('CO_SLIDER_FB_MAIL') . ': ' . $your_mail . '<br>';
        $body        .= \constant('CO_SLIDER_FB_SITE') . ': ' . $your_site . '<br>';
        $body        .= \constant('CO_SLIDER_FB_TYPE') . ': ' . $fb_type . '<br><br>';
        $body        .= \constant('CO_SLIDER_FB_TYPE_CONTENT') . ':<br>';
        $body        .= $fb_content;
        ...
        if ($ret) {
            \redirect_header('index.php', 3, \constant('CO_SLIDER_FB_SEND_SUCCESS'));
        }
        ...
        echo '<div style="text-align:center;width: 80%; padding: 10px; border: 2px solid #ff0000; color: #ff0000; margin-right:auto;margin-left:auto;">
            <h3>' . \constant('CO_SLIDER_FB_SEND_ERROR') . '</h3>
            </div>';
        $form = $feedback->getFormFeedback();
        $form->display();
