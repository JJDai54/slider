...
trait VersionChecks
{
    ...
    public static function checkVerXoops(\XoopsModule $module = null, $requiredVer = null)
    {
        ...
        if (\version_compare($currentVer, $requiredVer, '<')) {
            $success = false;
            $module->setErrors(\sprintf(\constant('CO_SLIDER_ERROR_BAD_XOOPS'), $requiredVer, $currentVer));
        }
    ...
    public static function checkVerPhp(\XoopsModule $module = null)
    {
        ...
        if (false !== $reqVer && '' !== $reqVer) {
            if (\version_compare($verNum, $reqVer, '<')) {
                $module->setErrors(\sprintf(\constant('CO_SLIDER_ERROR_BAD_PHP'), $reqVer, $verNum));
                $success = false;
            }
        }
     ...
     */
    public static function checkVerModule($sliderHelper, $source = 'github', $default = 'master')
    {
        $moduleDirName      = \basename(\dirname(\dirname(__DIR__)));
        $moduleDirNameUpper = \mb_strtoupper($moduleDirName);
        $update             = '';
            ...
            if (\function_exists('curl_init') && false !== ($curlHandle = \curl_init())) {
                ...
                if (false === $curlReturn) {
                    ...
                    if ('master' !== $latestVersionLink) {
                        $update = \constant('CO_SLIDER_NEW_VERSION') . $latestVersion;
                    }
                    ...
                    $moduleVersion = ($sliderHelper->getModule()->getInfo('version') . '_' . $sliderHelper->getModule()->getInfo('module_status'));
                    //"PHP-standardized" version
