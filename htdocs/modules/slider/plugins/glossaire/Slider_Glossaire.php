...
class Slider_Glossaire extends Slider_Plugin
{
var $options = array(
            'table'        => 'glossaire_categories',
            'fld_id'       => 'categoryID',
            'fld_pid'      => '',
            'fld_name'     => 'name',
            'fld_weight'   => 'weight',
...
$this->options['captionAll'] = _MB_SLD_GLOSSAIRE_ALL_CAT;
    ...
    if (in_array('glossaire_submit',$permsNames)){
    $mainMenu['submit']['url'] = $moduleUrl . "/submit.php";
    $mainMenu['submit']['lib'] = _MB_SLD_GLOSSAIRE_SUBMIT;
    }
    
    // --- Suggérer une définition
    if (in_array('glossaire_request',$permsNames)){
    $mainMenu['submit']['url'] = $moduleUrl . "/request.php";
    $mainMenu['submit']['lib'] = _MB_SLD_GLOSSAIRE_REQUEST;
    }
    
    // --- Chercher une définition
    if (in_array('glossaire_view',$permsNames)){
    $mainMenu['submit']['url'] = $moduleUrl . "/search.php";
    $mainMenu['submit']['lib'] = _MB_SLD_GLOSSAIRE_SEARCH;
    }
